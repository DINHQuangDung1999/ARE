...
class SHACGAE(Agent):
    ...
    def __init__(self, full_cfg, **kwargs):
        ...
        # --- GAE Parameters ---
        self.init_logit_actor_gaelambda = self.shac_config.get('logit_actor_gaelambda', 3.0)
        self.actor_loss_type = self.shac_config.get('actor_loss_type', 'fixed-gae')
        self.update_lambda_every = self.shac_config.get('update_lambda_every', 1)
        self.decoupleTD = self.shac_config.get('decoupleTD', False)
        ...
        # --- Actor GAE ---
        self.gaelambda_iterations = self.shac_config.get('lambda_iterations', 1)
        self.num_gaelambda_batches = self.shac_config.get('num_lambda_batches', 1)
        self.gaelambda_batchsize = self.horizon_len // self.num_gaelambda_batches
        self.logit_actor_gaelambda = nn.Parameter(torch.tensor(self.init_logit_actor_gaelambda, device=self.device, dtype=torch.float32))
        if self.actor_loss_type == 'adpt-gae':
            self.logit_actor_gaelambda_optim = OptimCls([self.logit_actor_gaelambda], **self.shac_config.get("actor_gaelambda_optim_kwargs", {}))
        elif self.actor_loss_type == 'time-adpt-gae':
            LambdaCls = getattr(models, self.network_config.actor_gaelambda)
            self.actor_gaelambda = LambdaCls(obs_dim, self.action_dim, **self.network_config.get("actor_gaelambda_kwargs", {}))
            actor_gaelambda_optim_params = itertools.chain(self.encoder.parameters(), self.actor_gaelambda.parameters())
            self.actor_gaelambda_optim = OptimCls(
                actor_gaelambda_optim_params,
                **self.shac_config.get("actor_gaelambda_optim_kwargs", {}),
            )
            self.actor_gaelambda_lr = self.critic_optim.defaults["lr"]
            self.actor_gaelambda.to(self.device)
        # --- Entropy ---
    ...
    def train(self):
        ...
        while self.agent_steps < self.max_agent_steps:
            ...
            with torch.no_grad():
                self.compute_target_values(self.entropy_in_targets)
                values_results = {
    ...
    def update_actor(self):
        ...
        with torch.no_grad():
            ...
            if self.shac_config.truncate_grads:
                if self.shac_config.get("actor_agc_clip", None) is not None:
                    ...
                    if self.actor_loss_type == 'adpt-gae':
                        nn.utils.clip_grad_norm_(self.logit_actor_gaelambda_optim.param_groups[0]["params"], self.shac_config.max_grad_norm)
        ...
        ##############################
        if self.actor_loss_type == 'adpt-gae':
            if self.epoch % self.update_lambda_every == 0:
                if self.entropy_in_return:
                    entropy_for_target_value = self.distr_ent if self.use_distr_ent else -1.0 * self.logprobs
                    if self.offset_by_target_entropy:
                        entropy_for_target_value = (entropy_for_target_value + abs(self.target_entropy)) * 0.5
                    if self.scale_by_target_entropy:
                        entropy_for_target_value = entropy_for_target_value * (1.0 / abs(self.target_entropy))
                
                    alpha = self.get_alpha(scalar=True) if self.with_autoent else self.entropy_coef
                lmda_Vnext = torch.zeros(self.num_envs, dtype=torch.float32, device=self.device)
                for _ in range(self.gaelambda_iterations):
                    total_lmda_loss = 0.
                    for i in reversed(range(self.horizon_len)):
                        if i % self.gaelambda_batchsize == self.gaelambda_batchsize - 1:
                            lmda_Vnext = lmda_Vnext.detach()
                            lmda_Vtilde = torch.zeros((self.gaelambda_batchsize, self.num_envs), dtype=torch.float32, device=self.device)
                            lmda = F.sigmoid(self.logit_actor_gaelambda)
                        rew = self.rew_buf[i] + alpha * entropy_for_target_value[i] if self.entropy_in_return else self.rew_buf[i]
                        lmda_Vnext = rew + self.gamma * ((1-self.done_mask[i]) * ((1-lmda) * self.next_values[i].detach() + lmda * lmda_Vnext)
                                                                + self.done_mask[i] * self.next_values[i].detach())

                        lmda_Vtilde[i%self.gaelambda_batchsize] = lmda_Vnext
                        if i % self.gaelambda_batchsize == 0:
                            z = self.encoder(self.obs_buf)
                            if isinstance(z, dict):
                                z = z['z'] if 'z' in z.keys() else z['obs']
                            pred_vs = self.critic(z[i : i + self.gaelambda_batchsize], return_type='all').detach()
                            lmda_loss = torch.stack([F.mse_loss(pred_v.squeeze(-1), lmda_Vtilde, reduction='mean') for pred_v in pred_vs])
                            lmda_loss = lmda_loss.mean()
                            total_lmda_loss += lmda_loss/self.num_gaelambda_batches
                            self.logit_actor_gaelambda_optim.zero_grad()
                            lmda_loss.backward()
                            self.logit_actor_gaelambda_optim.step()
                    results["lambda_loss"].append(total_lmda_loss.detach())
        if self.actor_loss_type == 'adpt-gae' or self.actor_loss_type == 'fixed-gae':
            results["lambda"].append(self.scalar_actor_gaelambda)
        ##############################
        with torch.no_grad():
    ...
    def compute_actor_loss(self):
        ...
        rew_buf = torch.zeros((self.horizon_len, self.num_envs), dtype=torch.float32, device=self.device) # local buffer that saves grad
        entropy_buf = torch.zeros((self.horizon_len, self.num_envs), dtype=torch.float32, device=self.device) # local buffer that saves grad
        with torch.no_grad():
            if self.obs_rms is not None:
        ...
        Vnext = torch.zeros(self.num_envs, dtype=torch.float32, device=self.device)
        Vtilde = torch.zeros((self.horizon_len, self.num_envs), dtype=torch.float32, device=self.device)

        lmda = F.sigmoid(self.logit_actor_gaelambda).detach()
        self.scalar_actor_gaelambda = lmda

        for i in reversed(range(self.horizon_len)):
            rew = rew_buf[i] + alpha * entropy_buf[i] if self.entropy_in_return else rew_buf[i]
            Vnext = rew + self.gamma * ((1-self.done_mask[i]) * ((1-lmda) * next_vs[i+1] + lmda * Vnext)
                                                    + self.done_mask[i] * next_vs[i+1])
            Vtilde[i] = Vnext
        returns = Vtilde[0]
    ...
    def compute_target_values(self, entropy_in_targets):
        if entropy_in_targets:
        ...
        if self.critic_method == 'one-step':
            ...
            Vnext = torch.zeros(self.num_envs, dtype=torch.float32, device=self.device)
            for i in reversed(range(self.horizon_len)):
                rew = self.rew_buf[i] + alpha * entropy[i] if entropy_in_targets else self.rew_buf[i]
                lmda = self.lam if self.decoupleTD else self.scalar_actor_gaelambda
                Vnext = rew + self.gamma * ((1-self.done_mask[i]) * ((1-lmda) * self.next_values[i] + lmda * Vnext)
                                                + self.done_mask[i] * self.next_values[i])
                self.target_values[i] = Vnext
        else:
