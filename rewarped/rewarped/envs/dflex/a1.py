...
class A1(WarpEnv):
    ...
    def __init__(self, num_envs=1, episode_length=1000, early_termination=True, **kwargs):
        ...
        self.action_scale = 1.0 # This works with stiffness = 5.0 and mode target position
    ...
    def create_articulation(self, builder):
        wp.sim.parse_urdf(
            os.path.join(self.asset_dir, "dflex/a1.urdf"),
            builder,
            floating=True,
            density=1000.0,
            # stiffness=0.0,
        ...
        # builder.joint_q[7:] = [-0.1, 0.8, -1.5, 0.1, 0.8, -1.5, -0.1, 0.6, -1.5, 0.1, 0.6, -1.5]
        builder.joint_q[:7] = [0.0, 0.0, 0.0, *wp.quat_from_axis_angle(wp.vec3(1.0, 0.0, 0.0), -math.pi * 0.5)]
        builder.joint_q[1] = 0.5  # start_height
    ...
    def compute_observations(self):
        ...
        obs_buf = [
            ...
            lin_vel,  # 5:8
            ang_vel,  # 8:11
            joint_q.view(self.num_envs, -1)[:, 7:],  # 11:23
            self.joint_vel_obs_scaling * joint_qd.view(self.num_envs, -1)[:, 6:],  # 23:35
            up_vec[:, 1:2],  # 35
    ...
    def compute_reward(self):
        # return
        up_reward = 0.1 * self.obs_buf[:, 35]
        heading_reward = self.obs_buf[:, 36]
        ...
        if early_termination:
            terminated = (self.obs_buf[:, 0] < self.termination_height) + (self.obs_buf[:, 35] <= 0.5)
...
if __name__ == "__main__":
    run_env(A1,  episode_length=500, early_termination=True, render=False, render_mode=None)
